...
# Add a selectbox to filter by Unit
units = df['unit'].unique().tolist()
selected_unit = st.sidebar.selectbox("Filter by Unit:", ['All Units'] + units)

# Filter the dataframe by the selected unit, if one is chosen
if selected_unit != 'All Units':
    df = df[df['unit'] == selected_unit]

# Add a placeholder option to the selectbox for the main summary
pages = ['Select an option...', 'Gender', 'Generation', 'Religion', 'Unit', 'Tenure']
...
if selected_page != 'Select an option...':
    ...
    if selected_page == 'Gender':
        ...
        st.title(f"Gender Summary (Filtered by {selected_unit})")
        ...
        bar_chart = alt.Chart(gender_counts).mark_bar().encode(
            ...
            title=f'Gender Distribution in {selected_unit} (Bar Chart)',
            width=600,
            height=400
        )
    ...
    if selected_page == 'Generation':
        ...
        # Title
        st.title(f"Generation Summary (Filtered by {selected_unit})")
        ...
        bar_chart = alt.Chart(generation_counts).mark_bar().encode(
            ...
            title=f'Generation Distribution in {selected_unit} (Bar Chart)',
            width=600,
            height=400
        )
    ...
    # Religion, Unit, Age, Tenure follow a similar structure...
